...
const CategoryHeader = ({ 
  ...
  const itemsToShow = isShowingSubcategories ? subcategories : otherCategories;
  const headingText = isShowingSubcategories ? t("SousCategorie") : t("AutresCatégories");
    ...
    <div className="relative">
      <div className="relative h-48 sm:h-56 lg:h-64 overflow-hidden bg-gradient-to-r rounded-t-lg">
        <div 
          ...
          style={{ backgroundImage: `url(${current.img_url || '/images/no-available.png'})` }}
        >
          ...
          <div className="relative z-10 h-full flex items-center justify-center">
              ...
              <div className="mx-auto mb-4 relative">
                <div className="w-20 h-20 sm:w-24 sm:h-24 lg:w-28 lg:h-28 mx-auto rounded-2xl overflow-hidden shadow-2xl bg-white/10 backdrop-blur-sm ring-4 ring-white/30 ring-offset-2 ring-offset-amber-400/50">
                  <Image
                    src={current.img_url ? cloudinaryOptimizer.thumbnail(current.img_url, 112) : '/images/no-available.png'}
            ...
            >
              {itemsToShow.map((it) => (
                isShowingSubcategories ? (
                  <button
                    key={it.name_search}
                    onClick={() => onSubCategoryChange(it.name_search === selectedSubCategory ? "" : it.name_search)}
                    className={`flex-shrink-0 group cursor-pointer my-0.5 p-3 sm:p-4 rounded-xl min-w-[100px] sm:min-w-[120px] transition-all duration-300 border ${selectedSubCategory === it.name_search ? "border-amber-300 bg-amber-50/50 dark:bg-amber-900/10" : "bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700"}`}
                  >
                    <div className="flex flex-col items-center space-y-2">
                      <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-lg overflow-hidden bg-white dark:bg-gray-700 shadow-[0_0_0_2px_rgba(212,184,20,0.25)]">
                        <Image
                          src={it.img_url ? cloudinaryOptimizer.thumbnail(it.img_url, 56) : '/images/no-available.png'}
                          alt={locale === "ar" ? it.name_ar : it.name}
                  ...
                  <Link
                    href={`/product_category/${it.name_search}`}
                    key={it.name_search}
      ...
      {/* {subcategories.length > 0 && (
        <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 hidden sm:block">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
...
const ProductCard = ({ item, index, locale, t, calculerPourcentageReduction, viewMode = "grid", toggleFavorite, favorites }) => {
  ...
  const getOptimizedImageUrl = (image, isHover = false) => {
    if (!image?.secure_url) return "/images/no-available.png";
